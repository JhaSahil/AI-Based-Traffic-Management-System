    ...
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        if(direction=='right'):
            if(len(vehicles[direction][lane])>1 and vehicles[direction][lane][self.index-1].crossed==0):   
                self.stop = vehicles[direction][lane][self.index-1].stop - vehicles[direction][lane][self.index-1].currentImage.get_rect().width - gap         # setting stop coordinate
            else:
...
def repeat():
    ...
    # reset light of previously green which turns yellow to no. of vehicle = 0
    for i in range(0,3):
